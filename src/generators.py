...
def card_number_generator(start, end):
    for num in range(start, end + 1):
        card_number = f"{num:16d}"  # Форматируем число, чтобы  оно было 16-значным
        formatted_card_number = " ".join([card_number [i:i+4] for i in range(0, 16, 4)]) # Разделяем строку на блоки по 4 цифры
    yield formatted_card_number  # Возвращаем отформатированный номер карты
